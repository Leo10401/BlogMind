...
const Competitions = () => {
  ...
  const fetchCompData = async () => {
    ...
    const ids = data.map(comp => comp._id);
    fetchParticipationData(ids);
  }

  const fetchParticipationData = async (comp_ids) => {
    const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/part/getall`);
    const data = res.data;
    const allParticipations = data.filter(participation => comp_ids.includes(participation.competition));
    const obj = {};
    allParticipations.forEach(participation => {
      if (obj[participation.competition]) {
        obj[participation.competition] += 1;
      } else {
        obj[participation.competition] = 1;
      }
    });
    setParticipationCount(obj);
  }
                ...
                <div class="py-8 flex flex-wrap md:flex-nowrap">
                  <div class=" md:mb-0 mb-6 flex-shrink-0 flex flex-col">
                    <span class="font-semibold title-font text-gray-700">
                      <div className="flex items-center justify-around bg-black h-10 rounded-lg shadow-md shadow-black/35 ">
                        <Link href={'/admin/manage-participation/' + compet._id} className="flex items-center justify-center  h-10 rounded-full bg-transparent text-white transition-transform ease-in-out duration-300 hover:-translate-y-1">
                          <svg
